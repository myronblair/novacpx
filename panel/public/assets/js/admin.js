...
(async () => {
  ...
  async function dashboard() {
    ...
    setTimeout(() => {
      const canvas = document.getElementById('dash-stats-chart');
      if (!canvas || !hist.length) return;
      const initChart = () => initStatsChart(canvas, hist);
      if (!window.Chart) {
...
<div class="stats-grid" style="margin-bottom:1.5rem">
  <div class="stat-card">
    ...
    <div class="stat-sub">Load: ${(s.cpu?.load || [0,0,0]).map(n=>n.toFixed(2)).join(' / ')}</div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-sub">${Nova.bytes((s.disk?.total||0)-(s.disk?.free||0))} used of ${Nova.bytes(s.disk?.total||0)}</div>
    <div class="mt-1">${Nova.progressBar(s.disk?.pct || 0)}</div>
  </div>
  <div class="stat-card">
    <div class="stat-label">Uptime</div>
    <div class="stat-value stat-green" style="font-size:.95rem;padding-top:.4rem">${s.uptime || '—'}</div>
    <div class="stat-sub">PHP ${v.php_version || '—'} · v${v.installed_version || '—'}</div>
  </div>
</div>
...
<div class="grid-2 gap-2" style="margin-bottom:1.5rem">
  ...
  <div class="card">
    <div class="card-header"><span class="card-title">NovaCPX</span></div>
    <div class="card-body">
      <table><tbody>
        <tr><td class="text-muted">Version</td><td><strong>${v.installed_version || '—'}</strong></td></tr>
        <tr><td class="text-muted">Branch</td><td><code>${v.git_branch || 'main'}</code></td></tr>
        <tr><td class="text-muted">Commit</td><td><code>${(v.git_commit||'—').substring(0,8)}</code>${v.git_dirty ? ' <span class="badge badge-yellow">dirty</span>' : ''}</td></tr>
        <tr><td class="text-muted">PHP</td><td>${v.php_version || '—'}</td></tr>
        <tr><td class="text-muted">OS</td><td style="font-size:.82rem">${v.os || '—'}</td></tr>
      </tbody></table>
      <div class="mt-2">
        <button class="btn btn-primary btn-sm" onclick="adminPage('updates')">Check Updates</button>
      </div>
    </div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="card-body">
    ${hist.length === 0
      ? '<p class="text-muted" style="text-align:center;padding:2rem">No history yet — collected every 5 minutes via cron.</p>'
      : '<canvas id="dash-stats-chart" height="70"></canvas>'}
  </div>
</div>`;
  }
